/**
 * HomeScreen.tsx
 * Premium branded home with real logo, no bg container, improved card layout.
 */

import type { Screen } from '../App';

interface Props {
  navigate: (s: Screen) => void;
}

export default function HomeScreen({ navigate }: Props) {
  return (
    <div
      className="flex flex-col min-h-full px-5 pb-10"
      style={{ background: '#F5F1E8', paddingTop: 'calc(env(safe-area-inset-top, 0px) + 48px)' }}
    >
      {/* ── Brand Header ─────────────────────────────── */}
      <div className="flex flex-col items-start mb-8">
        {/* Transparent logo — SVG preferred for crispness */}
        <img
          src="/logo/logo.svg"
          alt="Sri Vaishnav Constructions"
          className="mb-5"
          style={{ width: 80, height: 80, objectFit: 'contain' }}
          onError={e => {
            (e.currentTarget as HTMLImageElement).src = '/logo/logo.png';
          }}
        />

        <p
          className="text-[10px] font-semibold tracking-[0.22em] uppercase mb-1"
          style={{ color: '#C8A96A', fontFamily: 'Montserrat, sans-serif' }}
        >
          Sri Vaishnav Constructions
        </p>
        <h1
          className="text-[28px] font-bold leading-tight"
          style={{ color: '#3B2A1F', fontFamily: 'Montserrat, sans-serif' }}
        >
          Letter Studio
        </h1>
        <p
          className="text-[13px] mt-2 leading-relaxed"
          style={{ color: '#7A6A5A', fontFamily: 'Montserrat, sans-serif' }}
        >
          Professional branded documents — in seconds.
        </p>
      </div>

      {/* ── Gold rule ────────────────────────────────── */}
      <div
        className="w-10 h-[2px] mb-8 rounded-full"
        style={{ background: '#C8A96A' }}
      />

      {/* ── Entry Cards ──────────────────────────────── */}
      <div className="flex flex-col gap-3">

        {/* 1. Create with AI — primary dark card */}
        <button
          className="w-full rounded-2xl px-5 py-5 text-left transition-all active:scale-[0.98] active:opacity-90"
          style={{
            background: '#3B2A1F',
            boxShadow: '0 4px 20px rgba(59,42,31,0.25)',
          }}
          onClick={() => navigate('intake')}
        >
          <div className="flex items-center gap-4">
            {/* AI Sparkle icon */}
            <div
              className="flex items-center justify-center rounded-xl shrink-0"
              style={{ width: 44, height: 44, background: 'rgba(200,169,106,0.18)' }}
            >
              <svg width="22" height="22" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M12 2L13.09 8.26L19 9L13.09 9.74L12 16L10.91 9.74L5 9L10.91 8.26L12 2Z" fill="#C8A96A"/>
                <path d="M19 15L19.54 17.73L22 18L19.54 18.27L19 21L18.46 18.27L16 18L18.46 17.73L19 15Z" fill="#C8A96A" opacity="0.7"/>
                <path d="M5 3L5.36 4.82L7 5L5.36 5.18L5 7L4.64 5.18L3 5L4.64 4.82L5 3Z" fill="#C8A96A" opacity="0.5"/>
              </svg>
            </div>
            <div className="flex-1">
              <p
                className="text-[15px] font-semibold"
                style={{ color: '#F5F1E8', fontFamily: 'Montserrat, sans-serif' }}
              >
                Create with AI
              </p>
              <p
                className="text-[12px] mt-0.5 leading-snug"
                style={{ color: 'rgba(245,241,232,0.6)', fontFamily: 'Montserrat, sans-serif' }}
              >
                Describe your need — AI drafts a complete formal letter.
              </p>
            </div>
            {/* Chevron */}
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" style={{ opacity: 0.5, flexShrink: 0 }}>
              <path d="M9 18L15 12L9 6" stroke="#F5F1E8" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
            </svg>
          </div>
        </button>

        {/* 2. Upload & Convert */}
        <button
          className="w-full rounded-2xl px-5 py-5 text-left transition-all active:scale-[0.98] active:opacity-90"
          style={{
            background: '#FFFDF8',
            border: '1.5px solid #C8A96A',
            boxShadow: '0 2px 12px rgba(200,169,106,0.12)',
          }}
          onClick={() => navigate('upload')}
        >
          <div className="flex items-center gap-4">
            {/* Upload icon */}
            <div
              className="flex items-center justify-center rounded-xl shrink-0"
              style={{ width: 44, height: 44, background: 'rgba(200,169,106,0.1)' }}
            >
              <svg width="22" height="22" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M21 15V19C21 20.1 20.1 21 19 21H5C3.9 21 3 20.1 3 19V15" stroke="#C8A96A" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                <polyline points="17 8 12 3 7 8" stroke="#C8A96A" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                <line x1="12" y1="3" x2="12" y2="15" stroke="#C8A96A" strokeWidth="2" strokeLinecap="round"/>
              </svg>
            </div>
            <div className="flex-1">
              <p
                className="text-[15px] font-semibold"
                style={{ color: '#3B2A1F', fontFamily: 'Montserrat, sans-serif' }}
              >
                Upload &amp; Convert
              </p>
              <p
                className="text-[12px] mt-0.5 leading-snug"
                style={{ color: '#7A6A5A', fontFamily: 'Montserrat, sans-serif' }}
              >
                Paste a .docx or PDF — AI restructures it onto branded letterhead.
              </p>
            </div>
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" style={{ opacity: 0.35, flexShrink: 0 }}>
              <path d="M9 18L15 12L9 6" stroke="#3B2A1F" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
            </svg>
          </div>
        </button>

        {/* 3. Blank Letterhead */}
        <button
          className="w-full rounded-2xl px-5 py-5 text-left transition-all active:scale-[0.98] active:opacity-90"
          style={{
            background: '#FFFDF8',
            border: '1.5px solid rgba(200,169,106,0.35)',
            boxShadow: '0 2px 8px rgba(59,42,31,0.04)',
          }}
          onClick={() => navigate('blank')}
        >
          <div className="flex items-center gap-4">
            {/* Document icon */}
            <div
              className="flex items-center justify-center rounded-xl shrink-0"
              style={{ width: 44, height: 44, background: 'rgba(59,42,31,0.06)' }}
            >
              <svg width="22" height="22" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M14 2H6C4.9 2 4 2.9 4 4V20C4 21.1 4.9 22 6 22H18C19.1 22 20 21.1 20 20V8L14 2Z" stroke="#3B2A1F" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" opacity="0.7"/>
                <polyline points="14 2 14 8 20 8" stroke="#3B2A1F" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" opacity="0.7"/>
                <line x1="8" y1="13" x2="16" y2="13" stroke="#3B2A1F" strokeWidth="1.5" strokeLinecap="round" opacity="0.4"/>
                <line x1="8" y1="17" x2="13" y2="17" stroke="#3B2A1F" strokeWidth="1.5" strokeLinecap="round" opacity="0.4"/>
              </svg>
            </div>
            <div className="flex-1">
              <p
                className="text-[15px] font-semibold"
                style={{ color: '#3B2A1F', fontFamily: 'Montserrat, sans-serif' }}
              >
                Blank Letterhead
              </p>
              <p
                className="text-[12px] mt-0.5 leading-snug"
                style={{ color: '#7A6A5A', fontFamily: 'Montserrat, sans-serif' }}
              >
                Download an empty branded letterhead PDF instantly.
              </p>
            </div>
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" style={{ opacity: 0.35, flexShrink: 0 }}>
              <path d="M9 18L15 12L9 6" stroke="#3B2A1F" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
            </svg>
          </div>
        </button>

      </div>

      {/* ── Footer ───────────────────────────────────── */}
      <div className="mt-auto pt-10">
        <p
          className="text-center text-[11px]"
          style={{ color: 'rgba(122,106,90,0.45)', fontFamily: 'Montserrat, sans-serif', letterSpacing: '0.05em' }}
        >
          Sri Vaishnav Constructions · Letter Studio
        </p>
      </div>
    </div>
  );
}
